...
def getHeadLines():
    ...
    soup = BeautifulSoup(response.text, 'html.parser') #scrape bbc headlines
    headlines = soup.find('body').find_all('h3') #get all haedlines
    print(headlines)
    finallist =""
    for head in headlines:
        finallist +=head.text.strip() +"\n" #strip it so it shows just the text
    return finallist
...
def daily_report():
    try:
        to_email = "email@gmail.com" #Change me
        smtp_server = "smtp.gmail.com" #Don't change unless it is needed (check your smtp server address)
        smtp_port = 465
        my_email = "email@gmail.com" #Change me
        my_password = "password"  #Please note, if you have 2fa, you need to setup a specific password, google how to depending on what email service you are using
